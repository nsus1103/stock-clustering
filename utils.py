...
def get_price_dataframe(data):
    df = pd.DataFrame(data, columns=['symbol', 'name', 'date', 'close'])
    df['close'] = df['close'].astype(float)
    df = df.pivot_table(index=["symbol", "name"],
                        columns='date',
                        values='close')
...
def get_ticker_within_year_range(year_start, year_end):
    connection = psycopg2.connect(host=config.DB_HOST, database=config.DB_NAME, user=config.DB_USER,
                                  password=config.DB_PASS)
    cursor = connection.cursor(cursor_factory=psycopg2.extras.DictCursor)
    try:
...
def get_ticker_within_date_range(date_start, date_end):
    connection = psycopg2.connect(host=config.DB_HOST, database=config.DB_NAME, user=config.DB_USER,
                                  password=config.DB_PASS)
    cursor = connection.cursor(cursor_factory=psycopg2.extras.DictCursor)
    try:
        cursor.execute(f""" SELECT stock_id
                        FROM daily_price
                        GROUP BY stock_id
                        HAVING  MIN(dt)>={date_start} AND MAX(dt)<={date_end}
                        ORDER BY MIN(dt) DESC;""")
        ticker_ids = cursor.fetchall()
    except Exception as e:
        print(e)
...
def get_price_data_within_date_range(date_start=pd.to_datetime('2021-06-01 00:00:00'), date_end=pd.to_datetime('2021-06-10 00:00:00')):
    connection = psycopg2.connect(host=config.DB_HOST, database=config.DB_NAME, user=config.DB_USER,
                                  password=config.DB_PASS)
    cursor = connection.cursor(cursor_factory=psycopg2.extras.DictCursor)
    ...
    # ticker_ids = get_ticker_within_date_range(date_start, date_end)
    print("Retrieving data between:", dates)
    try:
        cursor.execute(f""" SELECT s.symbol,s.name, d.dt, d.close
                        FROM daily_price AS d
                        INNER JOIN stock AS s
                        ON d.stock_id = s.id
                        WHERE dt>(%s) AND dt<(%s)
                        ORDER BY s.symbol, d.dt
                        """, dates)
    ...
    pivot_data = get_price_dataframe(price_data)

    return pivot_data

def get_percent_change_within_date_range(date_start=pd.to_datetime('2021-06-01 00:00:00'), date_end=pd.to_datetime('2021-06-10 00:00:00')):
    connection = psycopg2.connect(host=config.DB_HOST, database=config.DB_NAME, user=config.DB_USER,
                                  password=config.DB_PASS)
    cursor = connection.cursor(cursor_factory=psycopg2.extras.DictCursor)
    ...
    print("Retrieving data between:", dates)
    try:
        cursor.execute(f"""SELECT s.symbol,s.name, d.dt, d.percent_change
                        FROM (SELECT stock_id, dt,close, 
						ROUND((close - LAG(close) OVER (PARTITION BY stock_id ORDER BY dt))/close, 3) AS percent_change
						FROM daily_price) AS d
                        INNER JOIN stock AS s
                        ON d.stock_id = s.id
                        WHERE dt>(%s) AND dt<(%s)
                        ORDER BY s.symbol, d.dt
                        """, dates)
        price_change_data = cursor.fetchall()
    except Exception as e:
        print(e)
        connection.rollback()
    ...
    pivot_data = get_price_dataframe(price_change_data)
...
# def main():
#
# tickers = get_ticker_within_year_range(2010, 2020)
# # tickers = [5]
#
